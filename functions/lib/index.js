...
const v1_1 = require("./engine/calculation/v1");
// [Stability Patch] App Check Visibility
const REGION = "asia-northeast3";
const ENFORCE_APP_CHECK = process.env.FUNCTIONS_EMULATOR !== "true";
firebase_functions_1.logger.info(`[BOOT] Region: ${REGION} | EnforceAppCheck: ${ENFORCE_APP_CHECK} | Emulator: ${process.env.FUNCTIONS_EMULATOR === "true"}`);
...
exports.generateReport = (0, https_1.onCall)({
    ...
    if (!allowedSex.includes(rawData.sex) || !allowedCalendar.includes(rawData.calendar)) {
        throw new https_1.HttpsError("invalid-argument", "지정된 성별 또는 달력 형식이 유효하지 않습니다.");
    }
    // calendar=lunar 일 때 isLeapMonth 필수 체크
    if (rawData.calendar === "lunar" && typeof rawData.isLeapMonth !== "boolean") {
        throw new https_1.HttpsError("invalid-argument", "음력 선택 시 윤달 여부(isLeapMonth)를 반드시 지정해야 합니다.");
    }
    const dateRegex = /^\d{4}-\d{2}-\d{2}$/;
    if (!dateRegex.test(rawData.birthDate)) {
        throw new https_1.HttpsError("invalid-argument", "생년월일 형식이 올바르지 않습니다 (YYYY-MM-DD).");
    }
    ...
    if (!timeUnknown) {
        ...
        if (!rawData.birthTime || !timeRegex.test(rawData.birthTime)) {
            throw new https_1.HttpsError("invalid-argument", "출생 시간 형식이 올바르지 않습니다 (HH:mm).");
        }
    ...
    try {
        // 2. 실계산 실행
        const calculation = (0, v1_1.calculateV1)(input);
        ...
        firebase_functions_1.logger.error("Report Generation Error:", error);
        if (error instanceof https_1.HttpsError)
            throw error;
        throw new https_1.HttpsError("internal", "분석 엔진 처리 중 오류가 발생했습니다.");
    }
});
