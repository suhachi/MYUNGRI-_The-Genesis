...
function calculatePillars(input) {
    ...
    if (input.calendar === 'lunar') {
        solarDate = (0, converter_1.convertToSolar)(year, month, day, input.isLeapMonth);
    }
    else {
    ...
    const solarDateObj = (0, time_1.parseYMDToUTCDate)(`${solarDate.year}-${String(solarDate.month).padStart(2, '0')}-${String(solarDate.day).padStart(2, '0')}`);
    ...
    if (!input.timeUnknown && input.birthTime) {
        forensic = (0, time_1.calculateTrueSolarTime)(solarDateObj, input.birthTime, input.timezone);
        // Apply day shift for Calendar Iljin?
        effectiveDate = (0, time_1.addDaysUTC)(solarDateObj, forensic.dayShift);
    ...
    const finalLunar = (0, converter_1.convertToLunar)(effectiveYear, effectiveMonth, effectiveDay);
    let yearPillar = (0, ganzhi_1.toHanjaGanji)(finalLunar.secha);
    const monthPillar = (0, ganzhi_1.toHanjaGanji)(finalLunar.wolgeon);
    const dayPillar = (0, ganzhi_1.toHanjaGanji)(finalLunar.iljin);
    ...
    // 315 deg is Ipchun.
    const ipchunDate = (0, astronomy_1.getSolarTermDate)(solarY, 315);
    ...
    yearPillar = {
        stem: ganzhi_1.STEMS_HANJA[stemIdx],
        branch: ganzhi_1.BRANCHES_HANJA[branchIdx],
        label: `${ganzhi_1.STEMS_HANJA[stemIdx]}${ganzhi_1.BRANCHES_HANJA[branchIdx]}`
    };
    ...
    if (!input.timeUnknown && forensic) {
        ...
        const hourBranch = ganzhi_1.BRANCHES_HANJA[branchIdx];
        ...
        const dayStemIdx = ganzhi_1.STEMS_HANJA.indexOf(dayPillar.stem);
        if (dayStemIdx !== -1) {
            const startStemIdx = ((dayStemIdx % 5) * 2) % 10;
            const hourStemIdx = (startStemIdx + branchIdx) % 10;
            hourPillar = {
                stem: ganzhi_1.STEMS_HANJA[hourStemIdx],
                branch: hourBranch,
                label: `${ganzhi_1.STEMS_HANJA[hourStemIdx]}${hourBranch}`
            };
        }
    }
    return {
