...
exports.getFallbackAnalysis = getFallbackAnalysis;
exports.assertReferenceOnly = assertReferenceOnly;
...
// ㅅㅈㅊ -> Metal
// ㅇㅎ -> Water
function getHangulInitialElement(char) {
    // Decompose Hangul to get initial consonant
    const code = char.charCodeAt(0);
    if (code < 0xAC00 || code > 0xD7A3)
        return null; // Not Hangul Syllable
    const initialOffset = Math.floor((code - 0xAC00) / 588);
    const initials = [
        'ㄱ', 'ㄲ', 'ㄴ', 'ㄷ', 'ㄸ', 'ㄹ', 'ㅁ', 'ㅂ', 'ㅃ', 'ㅅ', 'ㅆ', 'ㅇ', 'ㅈ', 'ㅉ', 'ㅊ', 'ㅋ', 'ㅌ', 'ㅍ', 'ㅎ'
    ];
    const initial = initials[initialOffset];
    if (['ㄱ', 'ㄲ', 'ㅋ'].includes(initial))
        return 'Wood';
    if (['ㄴ', 'ㄷ', 'ㄸ', 'ㄹ', 'ㅌ'].includes(initial))
        return 'Fire';
    if (['ㅁ', 'ㅂ', 'ㅃ', 'ㅍ'].includes(initial))
        return 'Earth';
    if (['ㅅ', 'ㅆ', 'ㅈ', 'ㅉ', 'ㅊ'].includes(initial))
        return 'Metal';
    if (['ㅇ', 'ㅎ'].includes(initial))
        return 'Water';
    return null;
}
function getFallbackAnalysis(char) {
...
function assertReferenceOnly(analysis) {
...
//# sourceMappingURL=fallbackPhonetic.js.map