...
exports.JOHU_RULES = void 0;
exports.applyJohuAdjustment = applyJohuAdjustment;
...
function applyJohuAdjustment(pillars, strength) {
    const monthBranch = pillars.month.branch;
    const rule = exports.JOHU_RULES[monthBranch];
    let delta = 0;
    const evidence = [];
    const needs = [];
    ...
    if (rule) {
        ...
        // Check presence
        const hasCritical = rule.critical.some(needed => allChars.some(char => (0, tenGod_1.getElement)(char) === needed));
        if (!hasCritical) {
    ...
    if (finalScore <= strengthScore_1.STRENGTH_THRESHOLDS.WEAK_MAX)
        finalVerdict = 'Weak';
    else if (finalScore >= strengthScore_1.STRENGTH_THRESHOLDS.STRONG_MIN)
        finalVerdict = 'Strong';
    else
...
//# sourceMappingURL=johuAdjustment.js.map