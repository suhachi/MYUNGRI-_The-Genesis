...
function calculateRelations(pillars) {
    ...
    for (let i = 0; i < activePillars.length; i++) {
        for (let j = i + 1; j < activePillars.length; j++) {
            ...
            const pair = s1 + s2;
            // Hab
            if (rules_1.CHEONGAN_HAB[pair]) {
                list.push({
                    type: 'Hab', subtype: 'CheonganHab',
                    pillars: [k1, k2],
                    value: rules_1.CHEONGAN_HAB[pair],
                    description: `${s1}${s2} 합화 ${rules_1.CHEONGAN_HAB[pair]}`
                });
            }
            // Chung
            if (rules_1.CHEONGAN_CHUNG[pair]) {
                list.push({
    ...
    for (let i = 0; i < activePillars.length; i++) {
        for (let j = i + 1; j < activePillars.length; j++) {
            ...
            const pair = b1 + b2;
            // YukHab
            if (rules_1.JIJI_YUKHAB[pair]) {
                list.push({
                    type: 'Hab', subtype: 'YukHab',
                    pillars: [k1, k2],
                    value: rules_1.JIJI_YUKHAB[pair],
                    description: `${b1}${b2} 육합 (${rules_1.JIJI_YUKHAB[pair]})`
                });
            }
            // Chung
            if (rules_1.JIJI_CHUNG[pair]) {
                list.push({
    ...
    if (pillars.hour)
        branchMap['hour'] = pillars.hour.branch;
    const branches = Object.values(branchMap);
    // Check each SamHab rule
    for (const rule of rules_1.JIJI_SAMHAB) {
    ...
    // Check BangHab
    for (const rule of rules_1.JIJI_BANGHAB) {
        const target = rule.key.split('');
        const matches = target.filter(t => branches.includes(t));
        if (matches.length === 3) {
            list.push({
    ...
    const sIdx = ganzhi_1.STEMS_HANJA.indexOf(dayStem);
    const bIdx = ganzhi_1.BRANCHES_HANJA.indexOf(dayBranch);
    if (sIdx >= 0 && bIdx >= 0) {
        ...
        const void1 = ganzhi_1.BRANCHES_HANJA[v1Idx];
        const void2 = ganzhi_1.BRANCHES_HANJA[v2Idx];
        ...
        for (const k of activePillars) {
            if (k === 'day')
                continue; // Day cannot be void by itself (it DEFINES void)
